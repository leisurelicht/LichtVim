local function git_key(gs, bufnr)
  ...
  map.set({"n", "v"}, "<leader>ga", ":Gitsigns stage_hunk<CR>", "Add Hunk",
          {buffer = bufnr})
  map.set("n", "<leader>gA", gs.stage_buffer, "Add Buffer", {buffer = bufnr})
  map.set({"n", "v"}, "<leader>gr", ":Gitsigns reset_hunk<CR>", "Reset Hunk",
          {buffer = bufnr})
  map.set("n", "<leader>gR", gs.reset_buffer, "Reset Buffer", {buffer = bufnr})
  map.set("n", "<leader>gu", gs.undo_stage_hunk, "Undo Stage Hunk",
          {buffer = bufnr})
  map.set("n", "<leader>gw", gs.preview_hunk, "Preview Hunk", {buffer = bufnr})
  map.set("n", "<leader>gb", function() gs.blame_line {full = true} end,
          "Show Blame Line", {buffer = bufnr})
  map.set("n", "<leader>gd", gs.diffthis, "Diff This", {buffer = bufnr})
  map.set("n", "<leader>gD", function() gs.diffthis("~") end, "Diff This?",
          {buffer = bufnr})
  map.set("n", "<leader>gt", gs.toggle_deleted, "Toggle Deleted",
          {buffer = bufnr})
